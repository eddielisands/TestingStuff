...
import it.sephiroth.android.library.imagezoom.test.easing.Cubic;
import it.sephiroth.android.library.imagezoom.test.easing.Easing;
import it.sephiroth.android.library.imagezoom.test.factory.GeneralServiceFactory;
import it.sephiroth.android.library.imagezoom.test.graphics.FastBitmapDrawable;
import it.sephiroth.android.library.imagezoom.test.runnable.ImageReceiver;
import it.sephiroth.android.library.imagezoom.test.runnable.ImageReceiverCallback;
...
import it.sephiroth.android.library.imagezoom.test.utils.IDisposable;
 ...
 * 
 */
public abstract class ImageViewTouchBase extends ImageView implements IDisposable, ImageReceiverCallback {
	...
	public static final float ZOOM_INVALID = 3f;
	...
	public void setImageBitmap( final Bitmap bitmap, Matrix matrix, float min_zoom, float max_zoom ) {
		LogController.log("call setImageBitmap");
		if ( bitmap != null )
...
//	}
	...
	public void updateToView(final Bitmap bitmap, final int position, final String url) {
		LogController.log("call updateToView");
		handler.post(new Runnable(){
			Matrix imageMatrix;

			@Override
			public void run() {	
				LogController.log("call updateToView run1");
				if (url != null) {
					LogController.log("url: " + url);
				}
				
				if (requestingUrl != null) {
					LogController.log("erquestUrl: " + requestingUrl);
				}
				
				if (bitmap != null) {
					LogController.log("bitmap: " + bitmap.toString());
				}

				if(url != null && requestingUrl != null && bitmap != null)
				{
					LogController.log("call updateToView run2");

					if(url.equals(requestingUrl))
					{
						LogController.log("call updateToView run3");
						...
						if( null != bitmap )
						{
							...
							setImageBitmap( bitmap, imageMatrix.isIdentity() ? null : imageMatrix, ZOOM_INVALID, ZOOM_INVALID );
						}
					}
				}
			}
