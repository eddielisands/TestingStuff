...
import com.fancl.iloyalty.easing.Cubic;
import com.fancl.iloyalty.easing.Easing;
...
import com.fancl.iloyalty.utils.IDisposable;
...
//	public static final float ZOOM_INVALID = -1f;
	...
	public void setImageBitmap( final Bitmap bitmap, Matrix matrix, float min_zoom, float max_zoom ) {
		...
		if ( bitmap != null )
		{
			...
			setImageDrawable( fastBitmapDrawable , matrix, min_zoom, max_zoom );
		}
		else
		{
	...
	private FastBitmapDrawable fastBitmapDrawable;
...
//	}
	...
	public void updateToView(final Bitmap bitmap, final int position, final String url) {
		LogController.log("call updateToView " + bitmap.getWidth() + " " + bitmap.getHeight());
		handler.post(new Runnable(){
			...
			public void run() {	
				...
				if (requestingUrl != null) {
					LogController.log("requestUrl: " + requestingUrl);
				}
				...
				if (bitmap != null) {
					...
					setRequestingUrl(handler, requestingUrl, imageFolder);
				}
